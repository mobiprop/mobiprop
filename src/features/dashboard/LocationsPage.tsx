...
import { MOCK_LOCATIONS, DISTRIBUTION_CONFIG, type PropertyLocation } from "./locations-data";
import { EditLocationModal, type LocationFormValues } from "./components/EditLocationModal";
...
function StatCard({ label, value, iconBg, icon }: StatCardProps) {
  return (
    <div className="flex-1 min-w-[200px] bg-white border border-[#e5e7eb] rounded-[14px] p-4 flex flex-col gap-[18px] h-[112px]">
      ...
      <p className="text-[24px] font-semibold text-[#1e4f86] leading-[28px]" style={poppins}>{value}</p>
    </div>
  );
}
...
function LocationListCard({ location, isSelected, pinColors, onSelect }: LocationListCardProps) {
  return (
    <button
      type="button"
      onClick={onSelect}
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex items-center gap-2.5">
          <span className="size-9 rounded-[10px] flex items-center justify-center shrink-0" style={{ backgroundColor: pinColors.bg }}>
            <MapPin size={18} style={{ color: pinColors.color }} />
          </span>
          <div className="flex flex-col">
            <p className="text-[14px] font-semibold text-[#0d2138]" style={mont}>{location.name}</p>
            <p className="text-[12px] text-[#6a7282]" style={mont}>{location.region}</p>
          </div>
        </div>
        <span className="h-[22px] px-2.5 rounded-full bg-[#d1fae5] flex items-center text-[11px] font-semibold text-[#10b981] shrink-0" style={mont}>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div className="border border-[#e5e7eb] rounded-[10px] py-2 flex flex-col items-center">
          <p className="text-[15px] font-bold text-[#0d2138]" style={poppins}>{location.properties}</p>
          <p className="text-[11px] text-[#6a7282]" style={mont}>Properties</p>
        </div>
        <div className="bg-[#f9fafb] rounded-[10px] py-2 flex flex-col items-center">
          <p className="text-[15px] font-bold text-[#10b981]" style={poppins}>{location.active}</p>
          <p className="text-[11px] text-[#6a7282]" style={mont}>Active</p>
        </div>
        <div className="bg-[#f9fafb] rounded-[10px] py-2 flex flex-col items-center">
          <p className="text-[15px] font-bold text-[#10b981]" style={poppins}>{location.sold}</p>
          <p className="text-[11px] text-[#6a7282]" style={mont}>Sold</p>
        </div>
      </div>

      <div className="flex items-center justify-between">
        <p className="text-[12px]" style={mont}>
          <span className="font-semibold text-[#1f2937]">{formatRevenue(location.revenue)}</span>
          <span className="text-[#6b7280]"> revenue</span>
        </p>
        <p className="text-[12px] text-[#6b7280]" style={mont}>{location.agents} agents</p>
      </div>
    </button>
  );
}
...
function BottomStatCard({ label, icon, value, children }: BottomStatCardProps) {
  return (
    <div className="bg-white border border-[#e5e7eb] rounded-[14px] p-4 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        {icon}
        <p className="text-[12px] text-[#6a7282]" style={mont}>{label}</p>
      </div>
      <p className="text-[22px] font-bold text-[#1e4f86]" style={mont}>{value}</p>
      {children}
    </div>
  );
}
...
export function LocationsPage({ role }: LocationsPageProps) {
  const [locations, setLocations] = useState<PropertyLocation[]>(MOCK_LOCATIONS);
  ...
  const distributionTotal = selected
    ? selected.distribution.houses + selected.distribution.apartments + selected.distribution.lots + selected.distribution.commercial
    : 0;

  function handleAddLocation(values: LocationFormValues) {
    const newLocation: PropertyLocation = {
      id: values.name.toLowerCase().replace(/\s+/g, "-") || `location-${Date.now()}`,
      name: values.name,
  ...
  function handleEditLocation(values: LocationFormValues) {
    ...
    setLocations((prev) =>
      prev.map((loc) =>
        loc.id === selected.id
          ? { ...loc, name: values.name, region: values.region, address: values.address, postalCode: values.postalCode }
          : loc,
      ),
    );
    setModalMode(null);
  ...
  function handleDelete() {
    if (!selected) return;
    if (typeof window !== "undefined" && !window.confirm(`Delete ${selected.name}? This action cannot be undone.`)) {
      return;
    }
    setLocations((prev) => {
    ...
    <div className="px-6 py-5 flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex flex-col gap-0.5">
          <h1 className="text-[20px] font-medium text-[#0d2138] leading-[32px]" style={poppins}>Property Locations</h1>
          <p className="text-[14px] font-medium text-[#6a7282]" style={mont}>Manage properties across multiple locations</p>
        </div>
        {canManage && (
          <button
            type="button"
      ...
      <div className="flex flex-col lg:flex-row gap-3.5">
        {/* Location list */}
        <div className="lg:w-[380px] lg:shrink-0 bg-white border border-[#e5e7eb] rounded-[14px] overflow-hidden flex flex-col">
          ...
          <div className="flex flex-col overflow-y-auto max-h-[820px]">
            ...
            {filtered.length === 0 && (
              <p className="px-4 py-6 text-center text-[12px] text-[#6a7282]" style={mont}>No locations found.</p>
            )}
          </div>
        </div>
        ...
        {selected && (
          <div className="flex-1 flex flex-col gap-3.5 min-w-0">
            ...
            <div className="bg-white border border-[#e5e7eb] rounded-[14px] p-5 flex items-start justify-between gap-4 flex-wrap">
              <div className="flex items-center gap-3.5">
                ...
                <div className="flex flex-col gap-1">
                  <p className="text-[16px] font-semibold text-[#0d2138]" style={mont}>{selected.name}</p>
                  <p className="text-[12px] text-[#6a7282] flex items-center gap-1.5" style={mont}>
                    <Navigation size={12} />
                    {selected.address}
                  </p>
                  <div className="flex gap-2 mt-1">
                    <span className="h-[22px] px-2.5 rounded-full bg-[#f3f4f6] flex items-center text-[11px] font-medium text-[#6b7280]" style={mont}>
                      {selected.region}
                    </span>
                    <span className="h-[22px] px-2.5 rounded-full bg-[#f3f4f6] flex items-center text-[11px] font-medium text-[#6b7280]" style={mont}>
                      {selected.postalCode}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-3.5">
              ...
              <div className="bg-white border border-[#e5e7eb] rounded-[14px] overflow-hidden flex flex-col">
                <div className="px-4 py-3.5 border-b border-[#f0f0f0] flex items-center gap-2">
                  <MapPin size={16} className="text-[#0d2138]" />
                  <p className="text-[14px] font-semibold text-[#0d2138]" style={mont}>Location Map</p>
                </div>
                <div className="relative flex-1 min-h-[220px] bg-gradient-to-br from-[#e8f4ea] via-[#eef6ec] to-[#fbf6e9]">
                  ...
                  <div className="absolute bottom-3 left-3 px-2.5 py-1 rounded-full bg-white/90 text-[11px] text-[#6b7280]" style={mont}>
                    {selected.coordinates.lat.toFixed(4)}, {selected.coordinates.lng.toFixed(4)}
                  </div>
                  <div className="absolute bottom-3 right-3 flex flex-col gap-1.5">
                    <span className="size-7 rounded-[8px] bg-white shadow flex items-center justify-center text-[#6b7280]">
                      <Plus size={14} />
                    </span>
                    <span className="size-7 rounded-[8px] bg-white shadow flex items-center justify-center text-[#6b7280]">
                      <Minus size={14} />
              ...
              <div className="bg-white border border-[#e5e7eb] rounded-[14px] p-4 flex flex-col gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[14px] font-semibold text-[#0d2138]" style={mont}>Property Distribution</p>
                </div>
                  ...
                  {DISTRIBUTION_CONFIG.map((item) => {
                    ...
                    return (
                      <div key={item.key} className="flex flex-col gap-1.5">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className="size-7 rounded-[10px] flex items-center justify-center shrink-0" style={{ backgroundColor: item.iconBg }}>
                              <Icon size={14} style={{ color: item.iconColor }} />
                            </span>
                            <p className="text-[13px] font-medium text-[#1f2937]" style={mont}>{item.label}</p>
                          </div>
                        ...
                        <div className="h-2 w-full rounded-full bg-[#f3f4f6]">
                          <div className="h-2 rounded-full" style={{ width: `${percent}%`, backgroundColor: item.barColor }} />
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3.5">
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="h-1.5 w-full rounded-full bg-[#f3f4f6]">
                    <div
                      ...
                      style={{ width: `${selected.properties > 0 ? (selected.active / selected.properties) * 100 : 0}%` }}
                    />
                  </div>
                  <p className="text-[11px] text-[#6a7282]" style={mont}>{selected.active} active</p>
                </div>
              </BottomStatCard>
              <BottomStatCard
                label="Revenue"
              ...
              >
                <p className="text-[11px] text-[#10b981]" style={mont}>↑ +{selected.growthPercent}% from last month</p>
              </BottomStatCard>
              <BottomStatCard
              ...
              >
                <p className="text-[11px] text-[#6a7282]" style={mont}>Per property</p>
              </BottomStatCard>
              <BottomStatCard
                label="Active Agents"
                icon={<Users size={14} className="text-[#6a7282]" />}
                value={String(selected.agents)}
              >
                <p className="text-[11px] text-[#6a7282]" style={mont}>Team members</p>
              </BottomStatCard>
            </div>
            ...
            <div className="bg-white border border-[#e5e7eb] rounded-[14px] p-4 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                {selected.activity.length === 0 && (
                  <p className="text-[12px] text-[#6a7282]" style={mont}>No recent activity for this location.</p>
                )}
                  ...
                  return (
                    <div key={item.id} className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-2.5">
                        <span className="size-7 rounded-[10px] flex items-center justify-center shrink-0 mt-0.5" style={{ backgroundColor: item.iconBg }}>
                          <Icon size={14} style={{ color: item.iconColor }} />
                        </span>
                        <div className="flex flex-col">
                          <p className="text-[13px] font-medium text-[#1f2937]" style={mont}>{item.title}</p>
                          <p className="text-[12px] text-[#6a7282]" style={mont}>{item.description}</p>
                        </div>
                      </div>
                      <p className="text-[11px] text-[#9ca3af] whitespace-nowrap shrink-0" style={mont}>{item.timeAgo}</p>
                    </div>
                  );
                })}
              </div>
      ...
      {modalMode === "add" && (
        <EditLocationModal onClose={() => setModalMode(null)} onSubmit={handleAddLocation} />
      )}
      {modalMode === "edit" && selected && (
        <EditLocationModal location={selected} onClose={() => setModalMode(null)} onSubmit={handleEditLocation} />
      )}
    </div>
  );
}
