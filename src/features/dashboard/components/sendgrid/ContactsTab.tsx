...
const RECIPIENT_BADGE: Record<EmailRecipientStatus, { bg: string; text: string }> = {
  SUBSCRIBED: { bg: "#dcfce7", text: "#16a34a" },
  UNSUBSCRIBED: { bg: "#f3f4f6", text: "#6a7282" },
  BOUNCED: { bg: "#fee2e2", text: "#dc2626" },
  REMOVED: { bg: "#f3f4f6", text: "#6a7282" },
};
...
function ListFormModal({ list, onClose }: { list: EmailListDto | null; onClose: () => void }) {
  ...
  async function handleSubmit() {
    if (!name.trim()) {
      toast.error(t("listFormModal.toasts.nameRequired"));
      return;
    }
    try {
      if (list) {
        await updateMutation.mutateAsync({ id: list.id, name, description, doubleOptIn });
        toast.success(t("listFormModal.toasts.updated"));
      } else {
  ...
  const afterCreatingLines = t("listFormModal.afterCreatingLines", { returnObjects: true }) as unknown as string[];
      ...
      <div className="relative z-10 flex w-full max-w-[540px] flex-col overflow-hidden rounded-[18px] bg-white shadow-xl sm:rounded-[14px]" onClick={(e) => e.stopPropagation()}>
        <div className="shrink-0 border-b border-[#e5e7eb] px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex flex-col gap-0.5">
              <p className="text-[17px] font-semibold text-[#0d2138]" style={poppins}>{list ? t("listFormModal.editTitle") : t("listFormModal.newTitle")}</p>
              <p className="text-[12px] text-[#6a7282]" style={mont}>
                {list ? t("listFormModal.editSubtitle") : t("listFormModal.newSubtitle")}
              </p>
            </div>
            <button type="button" onClick={onClose} aria-label={t("listFormModal.closeAria")} className="flex size-9 shrink-0 items-center justify-center rounded-[10px] text-[#6a7282] hover:bg-[#f3f4f6] hover:text-[#0d2138]">
              <X size={18} />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 px-5 py-4">
          <div className="flex flex-col gap-1.5">
            <span className="text-[13px] font-semibold text-[#0d2138]" style={mont}>{t("listFormModal.nameLabel")} <span className="text-[#dc2626]">*</span></span>
            <input
              value={name}
              onChange={(e) => setName(e.target.value)}
              disabled={list?.isSystem}
              placeholder={t("listFormModal.namePlaceholder")}
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-[13px] font-semibold text-[#0d2138]" style={mont}>{t("listFormModal.descriptionLabel")}</span>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              rows={3}
              placeholder={t("listFormModal.descriptionPlaceholder")}
          ...
          <div className="flex items-center justify-between gap-3 rounded-[12px] border border-[#e5e7eb] p-4">
            <div className="flex flex-col gap-0.5">
              <span className="text-[14px] font-semibold text-[#0d2138]" style={mont}>{t("listFormModal.doubleOptIn")}</span>
              <span className="text-[12px] text-[#6a7282]" style={mont}>{t("listFormModal.doubleOptInHint")}</span>
            </div>
            <Toggle checked={doubleOptIn} onChange={setDoubleOptIn} label={t("listFormModal.doubleOptIn")} />
          </div>

          {!list && (
            <div className="flex flex-col gap-2 rounded-[12px] bg-[#eff6ff] p-4">
              <span className="text-[13px] font-semibold text-[#1e4f86]" style={mont}>{t("listFormModal.afterCreating")}</span>
              {afterCreatingLines.map((line) => (
        ...
        <div className="flex shrink-0 items-center gap-2.5 border-t border-[#e5e7eb] px-5 py-4">
          ...
          >
            ...
            {list ? t("listFormModal.saveChanges") : t("listFormModal.createList")}
          </button>
        </div>
      </div>
    </div>
...
function MembersModal({ list, canManage, canExport, onClose }: { list: EmailListDto; canManage: boolean; canExport: boolean; onClose: () => void }) {
  ...
  async function handleAddManual() {
    if (!manualEmail.trim()) {
      toast.error(t("membersModal.toasts.emailRequired"));
      return;
    }
    try {
  ...
  async function handleRemove(member: EmailRecipientDto) {
    if (!window.confirm(t("membersModal.toasts.removeConfirm", { email: member.email, listName: list.name }))) return;
    try {
        ...
        <div className="shrink-0 border-b border-[#e5e7eb] px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex flex-col gap-0.5">
              ...
              <p className="text-[12px] text-[#6a7282]" style={mont}>
                {t("membersModal.memberCount", { count: list.memberCount })} · {list.listId}
              </p>
            </div>
            <div className="flex items-center gap-2">
              {canExport && (
                <a
                  ...
                  style={mont}
                >
                  <Download size={14} /> {t("membersModal.exportCsv")}
                </a>
              )}
              {canManage && (
                <button
                  ...
                  style={mont}
                >
                  <UserPlus size={14} /> {t("membersModal.addContact")}
                </button>
              )}
              <button type="button" onClick={onClose} aria-label={t("membersModal.closeAria")} className="flex size-9 shrink-0 items-center justify-center rounded-[10px] text-[#6a7282] hover:bg-[#f3f4f6] hover:text-[#0d2138]">
                <X size={18} />
              </button>
            </div>
          </div>
          ...
          <div className="shrink-0 border-b border-[#f3f4f6] bg-[#fafbfc] px-5 py-4 flex flex-col gap-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
              <div className="flex flex-1 flex-col gap-1">
                <span className="text-[11px] font-semibold text-[#6a7282]" style={mont}>{t("membersModal.fromCrm")}</span>
                <ContactPicker
                  value={crmContactId}
                  label={crmContactLabel}
                  onSelect={(id, label) => { setCrmContactId(id); setCrmContactLabel(label); }}
                  placeholder={t("membersModal.crmSearchPlaceholder")}
                />
              </div>
              <button
                type="button"
                ...
                style={mont}
              >
                {addCrmMutation.isPending ? t("membersModal.adding") : t("membersModal.addFromCrm")}
              </button>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
              {[
                [t("membersModal.emailRequiredLabel"), manualEmail, setManualEmail, "maria@example.com"],
                [t("membersModal.firstNameLabel"), manualFirst, setManualFirst, "María"],
                [t("membersModal.lastNameLabel"), manualLast, setManualLast, "García"],
              ...
              <button
                ...
                style={mont}
              >
                {addMutation.isPending ? t("membersModal.adding") : t("membersModal.add")}
              </button>
            </div>
          </div>
        )}
        ...
        <div className="shrink-0 border-b border-[#f3f4f6] px-5 py-3">
          <div className="relative max-w-[320px]">
            ...
            <input
              ...
              placeholder={t("membersModal.searchPlaceholder")}
        ...
        <div className="flex-1 overflow-y-auto">
          {membersQuery.isLoading ? (
            <div className="flex items-center justify-center gap-2 py-14 text-[#6a7282]">
              ...
              <span className="text-[13px]" style={mont}>{t("membersModal.loading")}</span>
            </div>
          ) : members.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-14">
              <Users size={26} className="text-[#c9d6e5]" />
              <p className="text-[13px] text-[#6a7282]" style={mont}>
                {search ? t("membersModal.emptyFiltered") : t("membersModal.emptyNone")}
              </p>
            </div>
          ) : (
            <table className="w-full min-w-[640px]">
              <thead>
                <tr className="border-b border-[#e5e7eb] bg-[#fafbfc]">
                  {[
                    t("membersModal.columns.contact"), t("membersModal.columns.phone"), t("membersModal.columns.source"),
                    t("membersModal.columns.status"), t("membersModal.columns.added"), "",
                  ].map((h) => (
                    ...
                    <tr key={member.id} className="border-b border-[#f3f4f6] last:border-b-0">
                      ...
                      <td className="px-5 py-3">
                        <span className="inline-flex rounded-[6px] px-2.5 py-1 text-[11px] font-medium" style={{ backgroundColor: badge.bg, color: badge.text, ...mont }}>
                          {t(`recipientStatus.${member.status}`)}
                        </span>
                      </td>
                      ...
                      <td className="px-5 py-3 text-right">
                        {canManage && (
                          <button
                            ...
                            title={t("membersModal.removeAria")}
...
export function ContactsTab({ canManage, canExport }: { canManage: boolean; canExport: boolean }) {
  ...
  async function handleDelete(list: EmailListDto) {
    setMenuFor(null);
    if (!window.confirm(t("contacts.deleteConfirm", { name: list.name }))) return;
    try {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col gap-0.5">
          <h2 className="text-[16px] font-semibold text-[#0d2138]" style={poppins}>{t("contacts.title")}</h2>
          <p className="text-[13px] text-[#6a7282]" style={mont}>{t("contacts.subtitle")}</p>
        </div>
        {canManage && (
          <div className="flex items-center gap-2.5">
            <button
              ...
              style={mont}
            >
              <Upload size={15} /> {t("contacts.importContacts")}
            </button>
            <button
              type="button"
              ...
              style={mont}
            >
              <Plus size={15} /> {t("contacts.newList")}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 xl:grid-cols-4">
        <MiniStat label={t("contacts.stats.totalContacts")} value={totals.contacts} sub={t("contacts.stats.acrossAllLists")} icon={<Users size={16} className="text-[#1e4f86]" />} iconBg="#e8f0fe" />
        <MiniStat label={t("contacts.stats.activeLists")} value={totals.lists} sub={t("contacts.stats.audienceSegments")} icon={<ListChecks size={16} className="text-[#6366f1]" />} iconBg="#e0e7ff" />
        <MiniStat label={t("contacts.stats.unsubscribed")} value={totals.unsubscribed} sub={t("contacts.stats.optedOut")} icon={<UserX size={16} className="text-[#6a7282]" />} iconBg="#f3f4f6" />
        <MiniStat label={t("contacts.stats.bounced")} value={totals.bounced} sub={t("contacts.stats.invalidAddresses")} icon={<AlertCircle size={16} className="text-[#dc2626]" />} iconBg="#fee2e2" />
      </div>
      ...
      <div className="relative overflow-visible rounded-[14px] border border-[#e5e7eb] bg-white">
        <div className="flex items-center justify-between gap-3 border-b border-[#f3f4f6] px-5 py-3.5">
          ...
          <span className="shrink-0 text-[12px] text-[#6a7282]" style={mont}>{t("contacts.count", { count: filtered.length })}</span>
        </div>

        {listsQuery.isLoading ? (
          <div className="flex items-center justify-center gap-2 py-14 text-[#6a7282]">
            <Loader2 size={16} className="animate-spin" />
            <span className="text-[13px]" style={mont}>{t("contacts.loading")}</span>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-2 py-14">
            <ListChecks size={26} className="text-[#c9d6e5]" />
            <p className="text-[13px] text-[#6a7282]" style={mont}>
              {search ? t("contacts.emptyFiltered") : t("contacts.emptyNone")}
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto overflow-y-visible pb-32">
            <table className="w-full min-w-[760px]">
              <thead>
                <tr className="border-b border-[#e5e7eb] bg-[#fafbfc]">
                  {[
                    t("contacts.columns.listName"), t("contacts.columns.description"), t("contacts.columns.contacts"),
                    t("contacts.columns.unsub"), t("contacts.columns.bounced"), t("contacts.columns.lastUpdated"), "",
                  ].map((h) => (
                    <th key={h} className="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-[#6a7282]" style={mont}>{h}</th>
                  ))}
                </tr>
                            ...
                            {menuFor === list.id && (
                              <div className="mobi-dropdown-menu absolute right-5 top-14 z-10 w-[180px] overflow-visible rounded-[10px] border border-[#e5e7eb] bg-white p-1.5 shadow-[0_12px_35px_rgba(15,23,42,0.16)]">
                                <button
                                  ...
                                  style={mont}
                                >
                                  <Upload size={13} /> {t("contacts.row.importIntoList")}
                                </button>
                                <button
                                  type="button"
                                  ...
                                  style={mont}
                                >
                                  <Pencil size={13} /> {t("contacts.row.editList")}
                                </button>
                                {!list.isSystem && (
                                  <button
                                    type="button"
                                    ...
                                    style={mont}
                                  >
                                    <Trash2 size={13} /> {t("contacts.row.deleteList")}
                                  </button>
                                )}
                              </div>
                            )}
